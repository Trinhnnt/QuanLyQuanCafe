...
BEGIN
    ...
    BEGIN
        ...
        RETURN 1 -- Trả về 1 để biết cập nhật thành công
    END
    
    RETURN 0 -- Trả về 0 nếu mật khẩu không đúng
END
GO
...
END
GO
-- Lấy danh sách chi tiết hóa đơn theo ngày và phân trang
CREATE PROC USP_GetListBillInfoByDateAndPage
@checkIn date, @checkOut date, @page int
AS 
BEGIN
    DECLARE @pageRows INT = 10
    ...
    FROM BillInfoShow 
    WHERE id NOT IN (SELECT TOP (@exceptRows) id FROM BillInfoShow ORDER BY id)
    ORDER BY id
END
GO
-- Đếm số lượng chi tiết hóa đơn theo ngày
CREATE PROC USP_GetNumBillInfoByDate
@checkIn date, @checkOut date
AS 
BEGIN
    SELECT COUNT(*)
...
GO
--------------------------------------------------
-- 8. STORED PROCEDURES: QUẢN LÝ LOG
--------------------------------------------------
-- Lấy danh sách log hoạt động theo ngày và phân trang
CREATE PROC USP_GetListActivityLogByDateAndPage
@checkIn date, @checkOut date, @page int
AS 
BEGIN
    DECLARE @pageRows INT = 10
    ...
    ;WITH ActivityLogShow AS(
        SELECT 
            al.Id,
            al.UserId AS [Tên đăng nhập],
            al.Action AS [Hành động],
            al.Description AS [Mô tả],
            al.EntityName AS [Đối tượng],
            al.EntityId AS [Mã đối tượng],
            al.LogTime AS [Thời gian],
            al.IPAddress AS [Địa chỉ IP],
            al.UserAgent AS [Trình duyệt]
        FROM dbo.ActivityLog AS al
        WHERE CAST(al.LogTime AS DATE) >= @checkIn 
            AND CAST(al.LogTime AS DATE) <= @checkOut
    )
    
    SELECT TOP (@selectRows) * 
    FROM ActivityLogShow 
    WHERE Id NOT IN (SELECT TOP (@exceptRows) Id FROM ActivityLogShow ORDER BY Id DESC)
    ORDER BY Id DESC
END
GO

-- Đếm số lượng log hoạt động theo ngày
CREATE PROC USP_GetNumActivityLogByDate
@checkIn date, @checkOut date
AS 
BEGIN
    SELECT COUNT(*)
...
--------------------------------------------------
-- 9. STORED PROCEDURES: BÁO CÁO THỐNG KÊ
--------------------------------------------------
